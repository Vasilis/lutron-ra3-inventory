...
function flattenTree(areas: Area[], devicesByArea: Map<string | null, Device[]>): AreaNode[] {
...
export function AreaTree({ areas, devicesByArea }: AreaTreeProps) {
  ...
  const nodes = useMemo(() => flattenTree(areas, devicesByArea), [areas, devicesByArea]);
        ...
        {nodes.map(({ area, depth, deviceCount }) => {
          ...
          return (
            ...
            >
              ...
              />
              <span className="flex-1 truncate">{area.Name ?? "(unnamed)"}</span>
              {deviceCount > 0 && (
                <span
                  className={cn(
